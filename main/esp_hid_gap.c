...
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst)
{
    ...
    /* 遍历列表中所有 UUID, 查找 HID 服务 UUID (0x1812) */
    if (uuid_d != NULL && uuid_len >= 2) {
        for (int i = 0; i + 1 < uuid_len; i += 2) {
            uint16_t u = uuid_d[i] + (uuid_d[i + 1] << 8);
            if (u == ESP_GATT_UUID_HID_SVC) {
                is_hid = true;
            }
            /* 保留第一个 UUID 用于显示 */
            if (i == 0) {
    ...
    /* 如果 UUID 没找到 HID, 也检查外观码 (HID: 0x03C0-0x03CB) */
    if (!is_hid && appearance >= 0x03C0 && appearance <= 0x03CB) {
        is_hid = true;
    }
    ...
    /* 如果 UUID 和外观码都没找到 HID, 也检查设备名称是否包含鼠标/键盘关键词 */
    if (!is_hid && adv_name_len > 0) {
        /* 转换为小写进行比较 */
        char name_lower[64] = {0};
        size_t copy_len = adv_name_len < 63 ? adv_name_len : 63;
        for (size_t i = 0; i < copy_len; i++) {
            name_lower[i] = tolower(name[i]);
        }
        if (strstr(name_lower, "mouse") || strstr(name_lower, "keyboard") ||
            strstr(name_lower, "mi silent") || strstr(name_lower, "mi bt") ||
            strstr(name_lower, "hid")) {
            is_hid = true;
        }
    }
...
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_AUTH_CMPL_EVT:
        if (!param->ble_security.auth_cmpl.success) {
            ESP_LOGE(TAG, "BLE GAP AUTH 失败: addr=" ESP_BD_ADDR_STR ", reason=0x%x",
                     ESP_BD_ADDR_HEX(param->ble_security.auth_cmpl.bd_addr),
                     param->ble_security.auth_cmpl.fail_reason);
        } else {
            ESP_LOGI(TAG, "BLE GAP AUTH 成功: addr=" ESP_BD_ADDR_STR ", dev_type=%d, auth_mode=%d",
                     ESP_BD_ADDR_HEX(param->ble_security.auth_cmpl.bd_addr),
    ...
    case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT: {
        ESP_LOGI(TAG, "BLE GAP CONN_PARAMS UPDATE: status=%d, min_int=%d max_int=%d latency=%d conn_int=%dms timeout=%d",
